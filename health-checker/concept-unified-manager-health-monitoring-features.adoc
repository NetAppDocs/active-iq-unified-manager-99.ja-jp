...
+
Storage Automation Store には、NetApp 認定の自動化ストレージ ワークフロー パックが含まれており、OnCommand Workflow Automation （WFA） で使用するために開発されています。パックをダウンロードして、WFA にインポートすれば実行できます。自動化されたワークフローは、Storage Automation Store で利用できます。
