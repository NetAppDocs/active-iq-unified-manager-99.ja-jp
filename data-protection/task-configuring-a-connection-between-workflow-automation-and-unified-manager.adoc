...
[NOTE]
====
「`WFA pack for managing Clustered Data ONTAP`」は WFA 5.1 に含まれているため、以前必要だったように NetAppStorage Automation Store からこのパックをダウンロードして WFA サーバーに個別にインストールする必要はありません。 https://community.netapp.com/["ONTAP を管理するための WFA パック"^]
